...
export default function CreatorProfile({ navigate }: Props) {
  ...
  return (
    <AuthLayout>
      <div style={{ width: "100%", maxWidth: "480px", paddingBottom: "4rem" }}>

        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "2rem" }}>
          <span style={{ fontFamily: "'Syne', sans-serif", fontSize: "20px", fontWeight: 800, color: "#fff" }}>My Profile</span>
          <span onClick={() => navigate("creator-dashboard")} style={{ fontSize: "12px", color: "#555", cursor: "pointer" }}>← Dashboard</span>
        </div>
        ...
        <div style={{ ...sectionStyle, display: "flex", flexDirection: "column", gap: "1rem" }}>
          ...
          <div>
            ...
            <input style={inputStyle} placeholder="e.g. Lifestyle, Beauty, Fitness" value={niche} onChange={e => setNiche(e.target.value)} />
          </div>
          <div>
          ...
          <div style={{ display: "flex", gap: "8px" }}>
            <div style={{ flex: 1 }}>
              ...
              <input style={inputStyle} placeholder="e.g. 22" type="number" value={age} onChange={e => setAge(e.target.value)} />
            </div>
            <div style={{ flex: 1 }}>
          ...
          <div>
            ...
            <textarea style={{ ...inputStyle, minHeight: "90px", resize: "vertical" }} placeholder="Tell brands about yourself..." value={bio} onChange={e => setBio(e.target.value)} />
          </div>
        </div>
        ...
        <div style={{ ...sectionStyle, display: "flex", flexDirection: "column", gap: "1rem" }}>
          ...
          <div>
            <label style={{ ...labelStyle, fontSize: "10px" }}>Audience Location</label>
        ...
        <div style={{ ...sectionStyle, display: "flex", flexDirection: "column", gap: "1rem" }}>
          ...
          <div style={{ display: "flex", gap: "8px" }}>
            <div style={{ flex: 1 }}>
              <label style={{ ...labelStyle, fontSize: "10px" }}>Post</label>
              <input style={inputStyle} placeholder="£" type="number" value={rates.post} onChange={e => setRates(r => ({ ...r, post: e.target.value }))} />
            </div>
            <div style={{ flex: 1 }}>
              <label style={{ ...labelStyle, fontSize: "10px" }}>Story</label>
              <input style={inputStyle} placeholder="£" type="number" value={rates.story} onChange={e => setRates(r => ({ ...r, story: e.target.value }))} />
            </div>
          </div>
          <div style={{ display: "flex", gap: "8px" }}>
            <div style={{ flex: 1 }}>
              <label style={{ ...labelStyle, fontSize: "10px" }}>Reel</label>
              <input style={inputStyle} placeholder="£" type="number" value={rates.reel} onChange={e => setRates(r => ({ ...r, reel: e.target.value }))} />
            </div>
            <div style={{ flex: 1 }}>
              <label style={{ ...labelStyle, fontSize: "10px" }}>Video</label>
              <input style={inputStyle} placeholder="£" type="number" value={rates.video} onChange={e => setRates(r => ({ ...r, video: e.target.value }))} />
            </div>
          </div>
          <div>
            <label style={{ ...labelStyle, fontSize: "10px" }}>UGC Only (no posting)</label>
            <input style={inputStyle} placeholder="£" type="number" value={rates.ugc} onChange={e => setRates(r => ({ ...r, ugc: e.target.value }))} />
          </div>
        </div>
        ...
        <div onClick={() => { setSaved(true); setTimeout(() => setSaved(false), 2000); }} style={{ padding: "14px", borderRadius: "8px", background: saved ? "#1a1a1a" : "#fff", color: saved ? "#555" : "#0a0a0a", border: saved ? "1px solid #222" : "1px solid #fff", fontSize: "13px", fontWeight: 600, textAlign: "center", cursor: "pointer", letterSpacing: "0.08em", textTransform: "uppercase", transition: "all 0.2s" }}>
