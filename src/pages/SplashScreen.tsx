import { useEffect, useState } from "react";

interface Props {
  onComplete: () => void;
}

const LOGO_PATHS = [
  { d: "M268.743988,141.763657 C268.770264,140.839050 268.796509,139.914444 268.968628,138.063904 C268.973663,119.750961 269.030243,101.437645 268.898254,83.125687 C268.885986,81.426476 267.676086,79.735893 267.022247,78.041290 C265.681519,78.851440 264.154022,79.460625 263.030609,80.504608 C256.130219,86.916985 249.380585,93.492241 242.448868,99.869919 C240.085922,102.043976 236.203033,103.322350 235.114822,105.897491 C234.163528,108.148582 236.246307,111.628464 236.836929,114.595688 C238.044830,120.663971 239.928818,127.158257 235.640747,132.438889 C232.546341,136.249557 228.330673,139.533447 223.978882,141.864670 C212.656128,147.930222 200.982910,153.341537 188.925125,159.271774 C188.532455,153.090378 188.180252,147.545990 187.788666,141.381638 C174.279053,153.046722 161.510452,164.071960 148.805481,175.042252 C163.134384,183.761902 177.085220,192.251465 191.036057,200.741043 C191.380646,200.313141 191.725250,199.885239 192.069839,199.457336 C191.270569,195.139252 190.471298,190.821167 189.658249,186.428650 C190.906189,186.077881 192.752045,185.654800 194.533447,185.043854 C218.178879,176.934647 240.343750,165.827255 260.791931,151.475250 C263.901611,149.292648 265.688232,145.225082 268.743988,141.763657z", fill: "#ffffff" },
  { d: "M128.041168,140.882126 C132.347198,142.000931 136.653229,143.119751 142.352478,144.600555 C127.064552,157.461166 112.648994,169.587921 98.233437,181.714676 C97.829193,181.414505 97.424942,181.114319 97.020691,180.814133 C97.020691,176.211365 97.020691,171.608582 97.008347,166.274246 C96.997238,165.362289 96.998489,165.181900 97.297333,164.707855 C97.730225,157.650772 97.973732,150.886917 97.947639,144.124069 C97.937180,141.414383 97.333435,138.706955 96.997574,135.727798 C96.986702,135.276840 96.977989,135.096573 96.969284,134.916306 C96.984940,133.558655 97.000595,132.201004 97.240967,130.232178 C97.276009,128.402283 97.086334,127.183540 96.896652,125.964798 C96.936661,123.801773 97.280693,121.592850 96.962341,119.483917 C95.788017,111.704567 100.091866,107.115173 105.801323,102.986748 C124.005127,89.823792 142.027573,76.408920 160.293884,63.334187 C165.844223,59.361343 171.889130,56.052246 177.863312,52.718365 C179.304489,51.914135 181.365662,52.220932 183.676743,52.472023 C185.237396,53.284367 186.262344,53.951061 187.288757,53.953350 C213.030014,54.010765 238.771545,54.022274 264.512543,53.920010 C265.679047,53.915379 266.841064,52.771313 268.005157,52.157959 C268.453339,52.830238 269.441528,53.868385 269.270355,54.121033 C265.476868,59.720032 262.086914,65.723610 257.535583,70.647736 C250.409851,78.357109 242.725525,85.602264 234.831161,92.535484 C225.652557,100.596611 216.044556,108.174164 206.526382,115.841782 C197.168274,123.380470 187.504074,130.548904 178.310654,138.279892 C167.147339,147.667404 156.346924,157.485947 145.369888,167.095627 C134.993423,176.179520 124.588501,185.230896 114.202187,194.303558 C108.661530,199.143433 103.133720,203.998001 97.201767,209.194946 C95.152985,204.950058 95.231705,201.142258 98.336990,198.241348 C109.033417,188.248886 119.956711,178.499100 130.805710,168.670242 C137.624146,162.492935 144.539169,156.418716 151.220428,150.097000 C152.323242,149.053528 152.911331,146.981888 152.942032,145.370270 C153.106766,136.719467 153.019852,128.063873 153.019852,119.753464 C164.937210,113.278221 176.578140,106.953163 188.107529,100.688721 C187.794525,107.413536 187.504791,113.638428 187.173111,120.764412 C199.833862,110.487228 211.899780,100.692886 224.050797,90.829468 C211.662857,84.303108 199.698669,77.999992 187.475739,71.560562 C187.475739,76.882881 187.475739,81.703476 187.475739,86.308678 C171.638748,95.721939 155.839188,105.207825 139.932022,114.509651 C135.203888,117.274460 128.984497,117.933220 126.518005,124.739204 C125.845863,130.034561 125.566727,134.858078 125.527977,139.683517 C125.524826,140.076309 127.164734,140.482285 128.041168,140.882126z", fill: "#ffffff" },
  { d: "M268.086823,142.029373 C265.688232,145.225082 263.901611,149.292648 260.791931,151.475250 C240.343750,165.827255 218.178879,176.934647 194.533447,185.043854 C192.752045,185.654800 190.906189,186.077881 189.658249,186.428650 C190.471298,190.821167 191.270569,195.139252 192.069839,199.457336 C191.725250,199.885239 191.380646,200.313141 191.036057,200.741043 C177.085220,192.251465 163.134384,183.761902 148.805481,175.042252 C161.510452,164.071960 174.279053,153.046722 187.788666,141.381638 C188.180252,147.545990 188.532455,153.090378 188.925125,159.271774 C200.982910,153.341537 212.656128,147.930222 223.978882,141.864670 C228.330673,139.533447 232.546341,136.249557 235.640747,132.438889 C239.928818,127.158257 238.044830,120.663971 236.836929,114.595688 C236.246307,111.628464 234.163528,108.148582 235.114822,105.897491 C236.203033,103.322350 240.085922,102.043976 242.448868,99.869919 C249.380585,93.492241 256.130219,86.916985 263.030609,80.504608 C264.154022,79.460625 265.681519,78.851440 267.022247,78.041290 C267.676086,79.735893 268.885986,81.426476 268.898254,83.125687 C269.030243,101.437645 268.973663,119.750961 268.580200,138.587204 C268.156769,140.083466 268.121796,141.056412 268.086823,142.029373z", fill: "#ffffff" },
];

export default function SplashScreen({ onComplete }: Props) {
  const [phase, setPhase] = useState<"fadein" | "glitch" | "fadeout">("fadein");
  const [glitchFrame, setGlitchFrame] = useState(0);
  const [logoVisible, setLogoVisible] = useState(false);
  const [textVisible, setTextVisible] = useState(false);

  useEffect(() => {
    const tLogo = setTimeout(() => setLogoVisible(true), 100);
    const tText = setTimeout(() => setTextVisible(true), 700);
    const t1 = setTimeout(() => setPhase("glitch"), 1300);
    const t2 = setTimeout(() => setPhase("fadeout"), 2400);
    const t3 = setTimeout(() => onComplete(), 3200);
    return () => [tLogo, tText, t1, t2, t3].forEach(clearTimeout);
  }, []);

  useEffect(() => {
    if (phase !== "glitch") return;
    let frame = 0;
    const interval = setInterval(() => {
      setGlitchFrame(++frame);
      if (frame > 18) clearInterval(interval);
    }, 50);
    return () => clearInterval(interval);
  }, [phase]);

  const isGlitching = phase === "glitch" && glitchFrame > 0 && glitchFrame <= 18;
  const intensity = Math.sin((glitchFrame / 18) * Math.PI);

  // Per-frame randomised glitch transforms
  const sliceY1 = 30 + (glitchFrame * 17) % 80;
  const sliceY2 = 100 + (glitchFrame * 31) % 70;
  const shiftX = glitchFrame % 2 === 0 ? -8 * intensity : 10 * intensity;
  const shiftX2 = glitchFrame % 2 === 0 ? 6 * intensity : -12 * intensity;
  const showNoise = isGlitching && glitchFrame % 3 !== 0;
  const showRedLayer = isGlitching && glitchFrame % 2 === 0;

  return (
    <div style={{
      position: "fixed", inset: 0, background: "#0a0a0a",
      display: "flex", alignItems: "center", justifyContent: "center",
      zIndex: 9999,
      opacity: phase === "fadeout" ? 0 : 1,
      transition: phase === "fadeout" ? "opacity 0.9s ease" : "none",
    }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Syne:wght@800&display=swap');
      `}</style>

      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 20 }}>
        <div style={{
          position: "relative",
          width: 120, height: 72,
          opacity: logoVisible ? 1 : 0,
          transform: logoVisible ? "scale(1)" : "scale(0.9)",
          transition: "opacity 0.7s ease, transform 0.7s cubic-bezier(0.16,1,0.3,1)",
        }}>
          {/* Base logo */}
          <svg
            viewBox="0 0 365 219"
            width="120" height="72"
            style={{
              display: "block",
              filter: isGlitching && glitchFrame % 4 === 0 ? "brightness(1.4)" : "none",
            }}
          >
            {LOGO_PATHS.map((p, i) => (
              <path key={i} fill={p.fill} d={p.d} />
            ))}
          </svg>

          {/* Red chromatic layer */}
          {showRedLayer && (
            <svg
              viewBox="0 0 365 219"
              width="120" height="72"
              style={{
                position: "absolute", top: 0,
                left: shiftX,
                opacity: 0.45 * intensity,
                mixBlendMode: "screen",
                clipPath: `inset(${sliceY1}% 0 ${100 - sliceY1 - 15}% 0)`,
              }}
            >
              {LOGO_PATHS.map((p, i) => (
                <path key={i} fill={p.fill === "#000000" ? "#000" : "#ff2200"} d={p.d} />
              ))}
            </svg>
          )}

          {/* Cyan chromatic layer */}
          {isGlitching && glitchFrame % 2 !== 0 && (
            <svg
              viewBox="0 0 365 219"
              width="120" height="72"
              style={{
                position: "absolute", top: 0,
                left: shiftX2,
                opacity: 0.35 * intensity,
                mixBlendMode: "screen",
                clipPath: `inset(${sliceY2}% 0 ${100 - sliceY2 - 12}% 0)`,
              }}
            >
              {LOGO_PATHS.map((p, i) => (
                <path key={i} fill={p.fill === "#000000" ? "#000" : "#00eeff"} d={p.d} />
              ))}
            </svg>
          )}

          {/* Slice displacement — whole logo shifts on certain frames */}
          {isGlitching && glitchFrame % 3 === 0 && (
            <svg
              viewBox="0 0 365 219"
              width="120" height="72"
              style={{
                position: "absolute", top: 0,
                left: shiftX * 1.5,
                opacity: 0.9,
                clipPath: `inset(${sliceY1}% 0 ${100 - sliceY1 - 8}% 0)`,
              }}
            >
              {LOGO_PATHS.map((p, i) => (
                <path key={i} fill={p.fill} d={p.d} />
              ))}
            </svg>
          )}

          {/* Noise bar */}
          {showNoise && (
            <div style={{
              position: "absolute",
              top: `${sliceY2}%`,
              left: 0, right: 0,
              height: `${4 + intensity * 6}px`,
              background: `rgba(255,255,255,${0.08 * intensity})`,
              pointerEvents: "none",
            }} />
          )}
        </div>

        <p style={{
          fontFamily: "'Syne', sans-serif",
          fontSize: 26,
          fontWeight: 800,
          color: "#fff",
          letterSpacing: "-0.02em",
          margin: 0,
          opacity: textVisible ? 1 : 0,
          transform: textVisible ? "translateY(0)" : "translateY(6px)",
          transition: "opacity 0.5s ease, transform 0.5s ease",
          filter: isGlitching && glitchFrame % 3 === 0 ? `translate(${shiftX * 0.3}px)` : "none",
        }}>
          FlipCollab
        </p>
      </div>
    </div>
  );
}