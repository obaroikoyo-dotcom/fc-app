...
              <div onClick={async () => {
  const { data: camp } = await supabase.from("campaigns").select("budget").eq("id", activeApplication.campaign_id).single();
  setCampaignBudget(parseInt(camp?.budget || "0") * 100);
  setPaymentApp(activeApplication);
  setShowPayment(true);
}} style={{ flex: 1, padding: "14px", borderRadius: "8px", background: "#fff", color: "#0a0a0a", fontSize: "13px", fontWeight: 600, textAlign: "center", cursor: "pointer", letterSpacing: "0.08em", textTransform: "uppercase" }}>Accept & Pay</div>
