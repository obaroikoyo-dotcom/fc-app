...
const loadFavourites = async () => {
  ...
  >
    Manage Subscription
      ...
      <div style={{ padding: "0 1.25rem" }}>
        ...
        {sectionCard(<>
          {settingsRow("Edit Profile", "Name, bio, industry, location, links", () => setSettingsSection("edit-profile"), "edit-profile")}
          {settingsRow("Connect Social Platforms", "Connect TikTok/Instagram to post creator content", () => setSettingsSection("manage-accounts"), "social")}
          {settingsRow("Get Verified", isVerified ? "Verified ✓" : verificationRequest ? "Request pending review" : "Request a verified badge", () => setSettingsSection("get-verified"), "verified")}
          {settingsRow("Industry & Content Needs", "Sectors and media formats you need", () => setSettingsSection("industry-selection"), "industry")}
          {settingsRow("Campaign Preferences", "Creator tier and target audience", () => setSettingsSection("campaign-preferences"), "campaign-prefs")}
          ...
          {settingsRow("Visibility", "Control what creators see", () => setSettingsSection("visibility"), "visibility")}
          {settingsRow("Share Profile", "Get your shareable brand link", () => setSettingsSection("share-profile"), "share", true)}
        </>)}

        {sectionHeader("FlipCollab Activity")}
        {sectionCard(<>
          {settingsRow("Favourited Creators", `${favouritedCreators.length} saved`, () => setSettingsSection("favourites"), "favourites")}
  ...
  const renderIndustrySelection = () => (
    <div style={{ minHeight: "100vh", background: "#0a0a0a", fontFamily: "'DM Sans', 'Helvetica Neue', sans-serif", paddingBottom: "8rem" }}>
      ...
      <div style={{ position: "sticky", bottom: "3.05rem", left: 0, right: 0, padding: "1rem 1.25rem", background: "#0a0a0a", borderTop: "1px solid #111" }}>
  ...
  <div style={{ minHeight: "100vh", background: "#0a0a0a", fontFamily: "'DM Sans', 'Helvetica Neue', sans-serif", paddingBottom: "6rem" }}>
    ...
    <div style={{ padding: "1.25rem", color: "#aaa", fontSize: "13px", lineHeight: "1.6", display: "flex", flexDirection: "column", gap: "1rem" }}>
      ...
      {[
        ...
        { t: "2. Information We Collect", b: "Your name, email, profile info, messages, and campaign content. Payment details are collected and verified directly by Stripe - we never see or store them ourselves." },
        { t: "3. How We Use It", b: "To run your account, match brands with creators, process payments, send you account and payment-related emails, resolve disputes, and meet our legal obligations." },
        ...
        { t: "5. Trusted Partners We Work With", b: "We run on a small number of established providers who only get the data they need to do their job: Supabase (our database and sign-in), Stripe (payments), and Vercel (hosting). We don't sell your data to anyone." },
        { t: "6. How Long We Keep Your Data", b: "For as long as your account is active. Deleted within 30 days of account deletion, except payment records, which UK law requires us to keep for 6 years. Photos and videos shared during a deal are cleared automatically once that deal wraps up or after a period of inactivity - your conversations themselves always stay, so both sides keep a record of what was agreed. Deleting your account or being restricted removes everything, including media, right away." },
        ...
        { t: "9. Keeping Your Data Secure", b: "Every connection to FlipCollab is encrypted, and payments are handled by Stripe, a global leader in payment security. If you ever notice anything suspicious on your account, let us know straight away." },
